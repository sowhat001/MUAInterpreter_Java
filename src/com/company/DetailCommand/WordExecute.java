...
public class WordExecute extends OperExecute
{
    public WordExecute(CmdExecute cmdExecute)
    {
        super(cmdExecute);
    ...
    public void execute(Data cmd) throws Exception
    {
        ...
        if (getDataSet.size() == this.needValue)
        {
            ...
            if (d1.getType() != WORD_TYPE || d2.getType() == LIST_TYPE)
            {
                throw new Exception("Type Error: Illegal type.");
            }
            else
            {
                cmdExecute.pushToExecuteStack(new Data(d1.toString() + d2.getLiteral()));
            }
        }
        else
        {
